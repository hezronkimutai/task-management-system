...
const TaskForm: React.FC<Props> = ({ open, onClose, onSubmit, initial = {}, users }) => {
  ...
  const [dueDate, setDueDate] = React.useState<string>(initial.dueDate ? String(initial.dueDate) : '');
  ...
  setDueDate(initial.dueDate ? String(initial.dueDate) : '');
  }, [initial, open]);

  const handleSubmit = () => {
    if (!title.trim()) return;
  // convert local datetime input to ISO if present
  const due = dueDate === '' ? undefined : new Date(dueDate).toISOString();
  onSubmit({ ...initial, title: title.trim(), description, status, priority, assigneeId: assigneeId === '' ? undefined : (assigneeId as number), dueDate: due });
  };
